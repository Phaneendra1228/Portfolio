...
// Enable smooth scroll to top on reload, handled gracefully after preloader dismissal
if ('history' in window) {
  window.history.scrollRestoration = 'auto'; // Allow browser to restore scroll position first
}

// Globally accessible smooth scroll-to-top handler
window.smoothScrollToTop = () => {
  ...
  // Perform smooth, hardware-accelerated scroll to top of Home section
...
// ===== PREMIUM CERTIFICATE VIEWER MODAL LOGIC =====
(function() {
  ...
  const spinner = document.getElementById('cert-modal-spinner');
  const viewport = document.getElementById('cert-render-viewport');
  const pdfCanvas = document.getElementById('cert-pdf-canvas');
  ...
  // Set up delegated click listener on certificates grid
  document.addEventListener('click', function(e) {
    ...
    const titleEl = card.querySelector('.cert-body h3');
    const orgEl = card.querySelector('.cert-org');
    
    const title = titleEl ? titleEl.textContent : 'Certificate';
    const org = orgEl ? orgEl.textContent : '';
    
    openModal(pdfUrl, title, org);
  });

  // Open Modal function
  async function openModal(url, title, org) {
    currentUrl = url;
    modalTitle.textContent = title;
    modalOrg.innerHTML = `<i class="fas fa-university"></i> ${org}`;
    ...
    // Show spinner & modal, hide content elements
    spinner.classList.remove('hidden');
    viewport.classList.remove('img-mode');
  ...
  async function renderPdfPage() {
    ...
    try {
      ...
      // We will render with high resolution using devicePixelRatio to make text crystal clear
      const pixelRatio = window.devicePixelRatio || 1;
      
      // Get base viewport
      const baseViewport = page.getViewport({ scale: 1.0 });
      
      // Calculate responsive width (fit 90% of modal-body width)
      const bodyEl = document.querySelector('.cert-modal-body');
      const bodyWidth = bodyEl ? bodyEl.clientWidth - 60 : 800;
      const targetScale = (bodyWidth / baseViewport.width) * currentZoom;
      
      const viewportScaled = page.getViewport({ scale: targetScale * pixelRatio });
      
      pdfCanvas.width = viewportScaled.width;
      pdfCanvas.height = viewportScaled.height;
      
      // Render canvas scaled down in CSS so it displays sharp
      pdfCanvas.style.width = (viewportScaled.width / pixelRatio) + 'px';
      pdfCanvas.style.height = (viewportScaled.height / pixelRatio) + 'px';
      ...
      if (err.name === 'RenderingCancelledException') {
        console.log("PDF render cancelled (expected during fast zooming)");
      } else {
  ...
  function loadImage(url) {
    viewerImg.onload = function() {
      ...
      viewport.classList.add('img-mode');
      spinner.classList.add('hidden');
      updateImageSize();
    };
  ...
  function updateImageSize() {
    if (viewerImg.style.display !== 'block') return;
    const bodyEl = document.querySelector('.cert-modal-body');
    const bodyWidth = bodyEl ? bodyEl.clientWidth - 60 : 800;
    viewerImg.style.width = (bodyWidth * currentZoom) + 'px';
  }
  ...
  // Handle Resize for rendering responsiveness
  let resizeTimeout;
